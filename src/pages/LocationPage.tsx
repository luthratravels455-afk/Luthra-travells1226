...
export const LocationPage: React.FC = () => {
  ...
  const [locData, setLocData] = useState<LocationItem | null>(null);
  ...
  return (
    <div className="bg-zinc-950 text-white min-h-screen pt-32 pb-24">
      <PageSEO
        title={locData.title}
        description={locData.meta_description}
      />
      ...
      <Container size="7xl" className="space-y-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-6 space-y-6">
            ...
            <h1 className="font-serif text-4xl sm:text-5xl font-extrabold text-white leading-tight">
              Taxi Service in {locData.city_name}
            </h1>

            <p className="text-zinc-300 text-sm sm:text-base leading-relaxed">
              {locData.meta_description}
            </p>
        ...
        {locData.popular_areas && locData.popular_areas.length > 0 && (
          <div className="bg-zinc-900/80 border border-zinc-800 rounded-3xl p-8 space-y-4">
            <h3 className="font-serif text-xl font-bold text-[#C9A227] flex items-center gap-2">
              <Navigation className="w-5 h-5" /> Taxi Coverage Near {locData.city_name}
            </h3>
            <div className="flex flex-wrap gap-3 pt-2">
              {locData.popular_areas.map((area: string, idx: number) => (
                <div key={idx} className="bg-zinc-950 border border-zinc-800 px-4 py-2 rounded-xl text-xs font-semibold text-zinc-200 flex items-center gap-1.5">
